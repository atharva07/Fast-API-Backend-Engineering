...
"""
    In FastAPI when the request is received by the server along with authentication token (JWT), it needs to resolve the dependency before executing the API Call
    received from the User. So Fast API Starts going down the dependency tree. 
    ...
    get_current_user --> get_current_user_id, this will decode token, get user ID. At this point we have authenticated identity. FastAPI will resolve get current user. 
    
    get_project_membership --> This will get resolved on the basis of project_id and current_user which we got from get_current_user method. This will check if the user
    has access to a resource (project). get_membership will return the membership of user with project. 
    ...
    So now, require_project_perission will check the permission is present in the Permissions for the particular role which user has. Here, we have role QA_ENGINEER and 
    required_permission as PROJECT_VIEW. So, it will check if the permission is present in set of permissions which QA has.
